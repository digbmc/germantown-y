...
- [Install Git](https://collectionbuilder.github.io/cb-docs/docs/software/git/) Git is the version control that Github runs on, and you'll use it to push and pull changes to the site from your local repository, along with branch creation. (rewrite?) <!-- I would just put in some more details about what it means to push and pull changes from the github repository to your local one. Also link to the github documentation on how to create a branch https://docs.github.com/en/get-started/start-your-journey/hello-world -->

- [Install Ruby](https://collectionbuilder.github.io/cb-docs/docs/software/ruby/). You need to have Ruby downloaded to run <!-- I would mention which version of Ruby --> 

- [Install ImageMagick and Ghostscript](https://collectionbuilder.github.io/cb-docs/docs/software/optional/). This software processes files in the archive <!-- What archive? -->
...
#### Using VS Code
1. Open an empty VS Code Window <!-- link to an overview of how to do this. Look through the VSCode documentation -->
...
4. Now in documents, there will be a folder titled germantown-y. Open it in VSCode <!-- give instructions on how to open folders in VScode -->
...
### Folders 
- _data: Data is where the metadata is stored, along with other CSV files that configure elements of the site.  <!-- make data lowercase -->
- _exhibits: Where exhibit posts are stored <!-- make Where lowercase -->
- _includes: this is where all the [features] are stored. <!--link to features -->  The subfolders are cb (information about collection builder), features, head (for including information in the `<head>` element), index (features used mostly on the homepage), and js (features that use [Javascript](https://www.w3schools.com/js/default.asp))
...
- _site: this is the folder that stores the output of pages generated by Jekyll. You'll notice that the folder is less bright <!--explain what less bright means in this case --> than the others, that's because _site is in the [.gitignore](https://git-scm.com/docs/gitignore) file. When the site builds, it doesn't track that folder. 
...
- _pages: files for each page on the site
  ...
  <!-- for the "look into this one" ones, ask the other fellows and have them help you define ths --> 
...
<!-- I'd get rid of this line " So then there is the structure of parent, url, children" -->
...
### Guide to layouts
Layouts are HTML files that format how pages look. They're HTML files. They include <!-- I'd define divs and includes --> divs and includes that format the content of a page. All of them have a liquid `{{ content }}` element somewhere, that takes the markdown from a page and places it in the layout.  

#### Site Layouts: 

<!-- I would make it either (not used on the sit) or Not currently used...make it consistent in this list -->


- browse: formatting for the archive section. 
- cloud: layout for word cloud of metadata subjects. Not currently used.
- data: layout for the data page (add data page to site!!) <!-- have you done this yet? -->
- default: Outer shell layout: Default serves as a layout for other layouts  <!-- why is this using two colons -->
